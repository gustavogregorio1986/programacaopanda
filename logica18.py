...
dados = {
    ...
    'origin': ['JFK', 'LAX', 'MIA']  # Aeroportos de origem
}
...
# Exibir o DataFrame
print(df_avioes[['callsign', 'origin', 'destination', 'arrival_time']])
...
for _, row in df_avioes.iterrows():
    if pd.notnull(row['arrival_time']):  # Verifica se não é NaT
        print(f"Voo {row['callsign']} vindo de {row['origin']} com destino {row['destination']} chegará às {row['arrival_time'].strftime('%H:%M')}.")
    else:
        print(f"Voo {row['callsign']} vindo de {row['origin']} com destino {row['destination']} está no solo.")
